...
import type { CampaignObjective, Niche } from "@/lib/mock-data";
import { nicheLabels } from "@/lib/mock-data";

export const Route = createFileRoute("/_app/campaigns/new")({
  component: NewCampaignPage,
});

const objectives: { id: CampaignObjective; label: string; description: string; icon: typeof Target }[] = [
...
function NewCampaignPage() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    toast.success("Campanha criada!", {
      description: `"${name}" entrou na fila de distribuição.`,
    });
    setTimeout(() => navigate({ to: "/campaigns" }), 800);
  };
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-6">
      ...
      <div>
        ...
        <p className="text-sm text-muted-foreground">Configure seu anúncio em poucos passos</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <section className="card-elevated p-6">
          ...
          <Field label="Nome da campanha *">
            <input
              ...
              placeholder="Ex: Lançamento Bot de Sinais"
              className={inputCls}
            />
          </Field>
        </section>
        ...
        <section className="card-elevated p-6">
          ...
          <div className="grid gap-4">
            <div className="rounded-xl border-2 border-dashed border-border p-6 text-center transition hover:border-primary/50">
              ...
              <p className="text-xs text-muted-foreground">MP4, MOV, JPG ou PNG (até 50MB)</p>
            </div>
            <Field label="Texto principal *">
              <textarea
                ...
                rows={3}
                placeholder="🚀 Texto chamativo para o seu anúncio..."
                className={inputCls}
              />
            </Field>
        ...
        <section className="card-elevated p-6">
          ...
          <div className="mt-5">
            <Field label={`Orçamento — ${budget} créditos (R$ ${budget.toFixed(2)})`}>
              <input
                type="range"
                min={10}
                max={2000}
        ...
        <div className="sticky bottom-20 z-10 flex flex-col-reverse gap-2 sm:bottom-4 sm:flex-row sm:justify-end">
          ...
          <button
            type="submit"
            className="rounded-xl gradient-primary px-6 py-2.5 text-sm font-semibold text-primary-foreground transition hover:brightness-110 hover:scale-[1.02] glow-primary"
          >
            Publicar campanha
          </button>
        </div>
