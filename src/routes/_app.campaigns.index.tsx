...
import { Search, Plus, Megaphone, Filter, Pause, Play, ArrowUpRight, Send } from "lucide-react";
...
function CampaignsPage() {
  ...
  const totalDms = filtered.reduce((a, c) => a + generateMetrics(c).dmsSent, 0);
  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="font-display text-[24px] font-bold tracking-tight md:text-[32px]">Campanhas</h1>
          <p className="text-[13px] text-muted-foreground">Gerencie e otimize todos os seus disparos</p>
        </div>
      ...
      <div className="tile overflow-hidden">
        <div className="flex flex-wrap items-center gap-1 border-b border-border/40 px-2 py-1.5 overflow-x-auto scroll-thin">
          {([
            { k: "all", l: "Todas" }, { k: "active", l: "Ativas" }, { k: "paused", l: "Pausadas" },
            { k: "draft", l: "Rascunhos" }, { k: "completed", l: "Concluídas" },
          ] as const).map((t) => (
            <button key={t.k} onClick={() => setFilter(t.k)}
              className={cn("inline-flex shrink-0 items-center gap-1.5 rounded-md px-2.5 py-1.5 text-[11.5px] font-semibold transition",
        ...
        <div className="flex flex-col gap-2 border-b border-border/40 p-3 sm:flex-row sm:items-center">
          <div className="relative flex-1">
            ...
            <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Buscar por nome da campanha…"
          ...
          <button className="hidden sm:inline-flex items-center gap-1.5 rounded-lg border border-border/60 bg-surface-1/60 px-3 py-1.5 text-[11.5px] font-semibold text-muted-foreground transition hover:text-foreground">
        ...
        <div className="grid grid-cols-3 gap-px bg-border/30">
          ...
          <SummaryCell label="DMs enviadas" value={compactNumber(totalDms)} accent />
            ...
            <table className="w-full text-[12px]">
              <thead className="bg-surface-1/40 text-[10px] uppercase tracking-wider text-muted-foreground">
                <tr>
                  ...
                  <th className="px-2 py-2.5 text-left font-semibold">Progresso</th>
                  <th className="px-2 py-2.5 text-right font-semibold">Entregues</th>
                    ...
                    <tr key={c.id} className="group border-t border-border/30 transition hover:bg-primary/[0.04]">
                      <td className="px-3 py-3">
                        <button onClick={(e) => { e.stopPropagation(); toggle(c); }}
                          ...
                          className={cn("flex h-7 w-7 items-center justify-center rounded-md border transition",
                            c.status === "active" ? "border-warning/40 bg-warning/10 text-warning hover:bg-warning/20" : "border-success/40 bg-success/10 text-success hover:bg-success/20",
                      ...
                      <td className="px-2 py-3 min-w-[140px]">
                        <div className="flex items-center justify-between text-[10px] tabular">
                          <span className="text-muted-foreground">{m.progressPct.toFixed(0)}%</span>
                          <span className="font-mono">{compactNumber(m.dmsSent)}/{compactNumber(m.dmTotal)}</span>
                        </div>
                        <div className="mt-1 h-1.5 overflow-hidden rounded-full bg-surface-3/60">
                          <div className="h-full gradient-primary" style={{ width: `${m.progressPct}%` }} />
                        </div>
                      </td>
                      <td className="px-2 py-3 text-right font-mono tabular">{compactNumber(m.dmsReceived)}</td>
                      ...
                      <div className="flex items-start gap-2.5">
                        ...
                        <div className="min-w-0 flex-1">
                          ...
                          <div className="mt-2 grid grid-cols-3 gap-2 text-[10.5px]">
                            <div><p className="text-muted-foreground">DMs</p><p className="tabular font-semibold">{compactNumber(m.dmsSent)}</p></div>
                            <div><p className="text-muted-foreground">Cliques</p><p className="tabular font-semibold">{compactNumber(m.clicks)}</p></div>
                            <div className="text-right"><p className="text-muted-foreground">CTR</p><p className="tabular font-semibold text-cyan">{m.ctr.toFixed(1)}%</p></div>
                          </div>
                          <div className="mt-2 flex items-center gap-2">
                            <div className="flex-1 h-1.5 overflow-hidden rounded-full bg-surface-3/60">
                              <div className="h-full gradient-primary" style={{ width: `${m.progressPct}%` }} />
                            </div>
                            <span className="text-[9.5px] tabular text-muted-foreground">{m.progressPct.toFixed(0)}%</span>
                          </div>
                        </div>
                      </div>
                    </Link>
...
function SummaryCell({ label, value, accent }: { label: string; value: string; accent?: boolean }) {
  return (
    <div className="bg-surface-1/40 px-3 py-2.5 sm:px-4 sm:py-3">
      <p className="text-[9.5px] font-bold uppercase tracking-wider text-muted-foreground">{label}</p>
      <p className={cn("mt-0.5 font-display text-base sm:text-lg font-bold tabular", accent && "text-gradient-primary")}>{value}</p>
    </div>
  );
}
...
function EmptyState() {
  return (
    <div className="flex flex-col items-center gap-3 p-12 text-center">
      ...
      <Link to="/campaigns/new" className="inline-flex items-center gap-1.5 rounded-lg gradient-primary px-3.5 py-1.5 text-[12px] font-semibold text-white">
        <Send className="h-3.5 w-3.5" /> Criar primeira campanha
      </Link>
    </div>
  );
}
